...
const API_URL = "https://andrewunoreyes.pythonanywhere.com/"; /* 'http://localhost:8000' http://127.0.0.1:8000/ or  'http://yourPythonAnywhereName.pythonanywhere.com/'*/
  ...
  export class APIService {
   ...
   deleteInvestment(investment_Pk){
    ...
    addNewStock(investment){
      const url = `${API_URL}/api/stocks/`;
      let jwtToken = localStorage.getItem('access');
      const headers = {Authorization: `JWT ${jwtToken}`};
      return axios.post(url, investment, {headers: headers});
   }
   updateStock(investment){
